...
class Misc extends Component {
    ...
    public static function printTree($data) {
        foreach ($data as $elm) {
            echo '<li data-id="' . $elm['id'] . '">' . $elm['name'];
            if (isset($elm['children'])) {
                echo '<ul>';
                self::printTree($elm['children']);
                echo '</ul>';
            }
            print '</li>';
        }
    ...
    public static function setFlash($type, $message) {
        return Yii::$app->session->setFlash('flash', json_encode(['message' => $message, 'type' => $type]));
    }
    ...
    public static function datetime($date = '') {
        return date('D, d M, Y H:i:a', self::timestamp($date));
    }

    public static function time($date = '') {
        return date('H:i:a', self::timestamp($date));
    }
    ...
    public static function delete_file($filename, $type) {
        if (self::file_exists($filename, $type)) {
            $file = Yii::$app->params['upload_path'][$type] . $filename;
