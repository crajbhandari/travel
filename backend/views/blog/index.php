               ...
               <table id = "datatable-buttons" class = "table table-striped table-bordered dt-responsive nowrap" style = "border-collapse: collapse; border-spacing: 0; width: 100%;">
                  ...
                  foreach ($blog as $post) :
                     ...
                     <tr>
                        ...
                        <td> <?php foreach ($post['translation'] as $trans):
                                echo (isset($trans['language_code']) && $trans['language_code'] == 'EN' ? trim($trans['title']) : '');
                            endforeach;
                            ?></td>
                        <td> <?php foreach ($post['translation'] as $trans):
                          echo (isset($trans['language_code']) && $trans['language_code'] == 'EN' ? trim($trans['category']) : '');
                            endforeach;
                            ?></td>
                        <td><?php echo (isset($post['visibility']) && $post['visibility'] == 1) ? 'Visible' : 'Hidden' ?></td>
                        <td> <?php foreach ($post['translation'] as $trans):
                               echo (isset($trans['language_code']) && $trans['language_code'] == 'EN' ? ucwords(trim($trans['author'])) : '');
                            endforeach;
                            ?></td>
                        <td>
                            <?php foreach ($post['translation'] as $trans): ?>
                               <a href = "<?=  Yii::$app->request->baseUrl; ?>/blog/post2/<?=\common\components\Misc::encrypt($trans['language_code'].'0'.$trans['id']); ?>">
                                  <span class = "lan">
                                     <?php echo \common\components\HelperLanguage::getSingleLanguageName($trans['language_code']) . '&nbsp;'; ?>
                              </span>
                               </a>
                            <?php
                            endforeach;
